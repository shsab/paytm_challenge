...
from utils.aws.aws_logging_handlers.validation import is_non_empty_string, is_positive_int, empty_str_err, bad_integer_err, ValidationRule
from utils.aws.aws_logging_handlers.tasks import Task, task_worker, STOP_SIGNAL
...
class S3Stream(BufferedIOBase):
    ...
    def __init__(self, bucket: str, key: str, *, chunk_size: int = DEFAULT_CHUNK_SIZE,
        ...
        self.current_file_name = "{}_{}.log".format(key, int(datetime.utcnow().strftime('%s')))
    ...
    def get_filename(self):
        ...
        """
        filename = "{}_{}.log".format(self.key, self.start_time)
